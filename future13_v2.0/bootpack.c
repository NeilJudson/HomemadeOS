...
void HariMain(void)
{
	...
	int mx, my, i, cursor_x, cursor_c, task_b_esp;				// cursor_x: 记录光标显示位置
	...
	load_tr(3 * 8);												// tr寄存器的作用是让CPU记住当前正在运行哪一个任务
	...
	tss_b.edi = 0;
	tss_b.es = 1 * 8;
	tss_b.cs = 2 * 8;											// 与bootpack.c相同的地址段，asmkfunc.nas中bootpack启动部分
	tss_b.ss = 1 * 8;
	tss_b.ds = 1 * 8;
	tss_b.fs = 1 * 8;
	tss_b.gs = 1 * 8;
